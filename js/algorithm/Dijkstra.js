...
export class Dijkstra {
  ...
  // Reset struktur data sebelum algoritma berjalan:
    ...
    *dijkstraGenerator(graph, startNode, endNode) {
    ...
    rekonstruksiPath(startNode, endNode) {

        // Jika endNode tak tercapai (jarak = ∞)
        if (this.dist.get(endNode.id) === Infinity) {
            return {
                nodes: [], edges: [],
        ...
        // Telusuri mundur dari endNode ke startNode
        while (current && current.id !== startNode.id) {
            nodes.unshift(current);          // sisipkan di depan
            const prevData = this.prev.get(current.id);
            if (!prevData) break;
            edges.unshift(prevData.edge);    // edge di depan
            current = prevData.node;         // mundur ke node sebelumnya
        }
        nodes.unshift(startNode); // tambahkan startNode di awal

        return {
            nodes,
            edges,
            totalDistance: this.dist.get(endNode.id),
            found: true,
            nodeLabels: nodes.map(n => n.label),
        };
    }

    // ── SNAPSHOT TABEL JARAK ──
    // Untuk panel info saat algoritma berjalan. O(V)
    getDistMap() {
        return new Map(this.dist);
    }
}
